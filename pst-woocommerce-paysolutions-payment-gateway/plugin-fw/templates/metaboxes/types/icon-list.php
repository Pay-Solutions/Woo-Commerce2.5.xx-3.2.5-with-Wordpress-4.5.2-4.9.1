...
$current_icon = PST_Icon()->get_icon_data( $current_options['icon'] );
$current_icon = PST_Icon()->get_icon_data( $std['icon'] );

$options['icon'] =  PST_Plugin_Common::get_icon_list();
...
<div id="<?php echo $id ?>-container" <?php if ( isset( $deps ) ): ?>data-field="<?php echo $id ?>" data-dep="<?php echo $deps['ids'] ?>" data-value="<?php echo $deps['values'] ?>" <?php endif ?>class="select_icon rm_option rm_input rm_text">
    ...
    <div class="option">
        ...
        <div class="input_wrapper custom_icon_wrapper upload" style="clear:both;">
            ...
            <input type="button" value="<?php _e( 'Upload', 'pst' ) ?>" id="<?php echo $id; ?>-custom-button" class="upload_button button" />
    ...
    <div class="description">
        ...
        <?php if( $std['select'] == 'custom' ) : ?>
            <?php printf( __( '(Default: %s <img src="%s"/>)', 'pst' ), $options['select']['custom'], $std['custom'] ) ?>
        <?php else: ?>
            <?php printf( __( '(Default: <i %s></i> )', 'pst' ), $current_icon  ) ?>
        <?php endif; ?>
    </div>
