...
//$categories = pst_get_model('cpt_unlimited')->get_setting( 'categories', $post->ID );
?>
...
<ul id="<?php echo $id ?>-extra-images" class="slides-wrapper extra-images ui-sortable clearfix" style="">
    ...
    <li>
        <a href="#">
            <?php pst_image( "id=$image_id&size=admin-post-type-thumbnails" );//echo wp_get_attachment_image( $image_id, 'admin-post-type-thumbnails' ); ?> 
            <input type="hidden" name="<?php echo $name ?>[]" value="<?php echo esc_attr( $image_id ) ?>" />
        </a>
        <a href="#" title="<?php _e( 'Delete image', 'pst' ) ?>" class="delete">X</a>
    </li>   
    <?php endforeach; endif; ?>
</ul>         
<a href="#" class="button-secondary upload-extra-images" id="<?php echo $id ?>-upload-extra-images"><?php _e( 'Upload new images', 'pst' ) ?></a>    
</div>
<script type="text/javascript">
    jQuery(document).ready(function($){
        $('#<?php echo $id ?>-upload-extra-images').on( 'click', function(){
            ...
            window.send_to_editor = function(html) {
            	...
            	$.post(ajaxurl, data, function(response) {
                    ...
                    $('#<?php echo $id ?>-extra-images.slides-wrapper').append('<li><a href="#"><img src="'+thumburl+'" width="140" height="100" /> <input type="hidden" name="<?php echo $name ?>[]" value="'+image_id+'" /></a><a href="#" title="<?php echo addslashes( __( 'Delete image', 'pst' ) ) ?>" class="delete">X</a></li>');
                });
        ...
        $('#<?php echo $id ?>-extra-images a.delete').on( 'click', function(){ 
            if ( confirm( "<?php _e( 'Are you sure you want to remove this image?', 'pst' ) ?>" ) ) {
                $(this).parent().remove();    
            }
