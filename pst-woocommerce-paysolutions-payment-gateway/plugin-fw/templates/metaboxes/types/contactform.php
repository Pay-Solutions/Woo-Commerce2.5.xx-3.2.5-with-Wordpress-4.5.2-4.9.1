...
$types = array(
    'text'     => __( 'Text Input', 'pst' ),
    'checkbox' => __( 'Checkbox', 'pst' ),
    'select'   => __( 'Select', 'pst' ),
    'textarea' => __( 'Textarea', 'pst' ),
    'radio'    => __( 'Radio Input', 'pst' ),
    'password' => __( 'Password Field', 'pst' ),
    'file'     => __( 'File Upload', 'pst' ),
);
...
$options["select"]=array(
    'icon'   => __( 'Theme Icon', 'pst' ),
    'custom' => __( 'Custom Icon', 'pst' ),
    'none'   => __( 'None', 'pst' )
);

$options["icon"] = PST_Plugin_Common::get_awesome_icons();
...
<p class="field-row">
    <a href="" class="button-secondary add-items"><?php _e( 'Add field', 'pst' ) ?></a>
...
<div class="contactform_items panel" id="panel_form">
    <?php while ( $index <= count( $value ) ): ?>
        <div class="contactform_item closed">
            <h3>
                <button type="button" class="remove_item button" rel=""><?php _e( 'Remove', 'pst' ) ?></button>
                <div class="handlediv" title="<?php _e( 'Click to toggle', 'pst' ) ?>"></div>
                <strong><?php echo $value[$index]['title'] ?> <?php pst_string( '(', $types[$value[$index]['type']], ')' ) ?></strong>
            ...
            <div class="inside">
                ...
                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_title_<?php echo $index ?>"><?php _e( 'Title Field', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Insert the title for the field.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_data_name_<?php echo $index ?>"><?php _e( 'Data Name', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'REQUIRED: Field identification name to be entered into email body. <strong>Note:</strong>Use only lowercase characters and underscores.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox select clearfix text-field-type">
                    <label for="<?php echo $id ?>_type_<?php echo $index ?>"><?php _e( 'Type field', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Select the type for this field.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox checkbox clearfix deps_checkbox deps">
                    <label for="<?php echo $id ?>_already_checked_<?php echo $index ?>"><?php _e( 'Checked', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Select this option if you want this field appears as already checked.', 'pst' ) ?></span>
                    </p>
                </div>

                <div id="<?php echo $id ?>_addoptions" class="the-metabox addoptions clearfix deps_radio deps_select deps">
                    <label for=""><?php _e( 'Add options ', 'pst' ) ?></label>
                    <a href="#" class="add-field-option button-secondary" data-index="<?php echo $index ?>"><?php _e( 'Add option', 'pst' ) ?></a><br /><br />
                    <?php foreach ( $value[$index]['options'] as $key => $option ) : ?>
                        <p class="option">
                            <label><input type="radio" name="<?php echo $name ?>[<?php echo $index ?>][option_selected]" value="<?php echo esc_attr( $key ) ?>"<?php checked( $value[$index]['option_selected'], $key ) ?> /> <?php _e( 'Selected', 'pst' ) ?>
                            </label>
                            <input type="text" name="<?php echo $name ?>[<?php echo $index ?>][options][]" value="<?php echo $option ?>" style="width:200px" />
                            <a href="#" class="del-field-option button-secondary"><?php _e( 'Delete option', 'pst' ) ?></a>
                        </p>
                    <?php endforeach; ?>
                </div>

                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_error_<?php echo $index ?>"><?php _e( 'Error Message', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Insert the error message for validation.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox checkbox clearfix">
                    <label for="<?php echo $id ?>_required_<?php echo $index ?>"><?php _e( 'Required', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Select this option if it must be required.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox checkbox clearfix">
                    <label for="<?php echo $id ?>_is_email_<?php echo $index ?>"><?php _e( 'Email', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Select this option if the email must be valid.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox checkbox clearfix">
                    <label for="<?php echo $id ?>_reply_to_<?php echo $index ?>"><?php _e( 'Reply To', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Select this if it is the email you can reply to.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_class_<?php echo $index ?>"><?php _e( 'Class', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Insert additional class(es) (separated by commas) for more personalization.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_icon_<?php echo $index ?>"><?php _e( 'Icon', 'pst' ) ?></label>
                    ...
                    <div class="option">
                        ...
                        <div class="input_wrapper custom_icon">
                            ...
                            <input type="button" name="<?php echo $name ?>[<?php echo $index ?>][custom]-button" value="<?php _e( 'Upload', 'pst' ) ?>" id="<?php echo $id ?>_icon_<?php echo $index ?>[custom-icon]-button" class="upload_button button" />

                            <div class="upload_img_preview" style="margin-top:10px;">
                                <?php
                                $file = $current_options['custom'];
                                if ( preg_match( '/(jpg|jpeg|png|gif|ico)$/', $file ) ) {
                                    echo __('Image preview', 'pst') . ': ' . "<img src=\"" . PST_CORE_ASSETS_URL . "/images/sleep.png\" data-src=\"$file\" />";
                                }
                                ?>
                            </div>
                        ...
                        <span class="desc inline"><?php _e( 'Insert an icon for more personalization.', 'pst' ) ?></span>
                    </p>
                </div>

                <div class="the-metabox text clearfix">
                    <label for="<?php echo $id ?>_width_<?php echo $index ?>"><?php _e( 'Width', 'pst' ) ?></label>
                    ...
                    <p>
                        ...
                        <span class="desc inline"><?php _e( 'Set field length.', 'pst' ) ?></span>
                    </p>
                </div>
            </div>
        </div>
...
<div class="contactform_item closed" id="stamp_form" style="display:none;">
    ...
    <div class="inside">
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_title"><?php _e( 'Title Field', 'pst' ) ?></label>
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_data_name"><?php _e( 'Data Name', 'pst' ) ?></label>
        ...
        <div class="the-metabox select clearfix text-field-type">
            <label for="<?php echo $id ?>_type"><?php _e( 'Type field', 'pst' ) ?></label>
        ...
        <div class="the-metabox checkbox clearfix deps_checkbox deps">
            <label for="<?php echo $id ?>_already_checked"><?php _e( 'Checked', 'pst' ) ?></label>
        ...
        <div id="<?php echo $id ?>_addoptions" class="the-metabox addoptions clearfix deps_radio deps_select deps">
            <label for=""><?php _e( 'Add options ', 'pst' ) ?></label>
            <a href="#" class="add-field-option button-secondary"><?php _e( 'Add option', 'pst' ) ?></a><br /><br />

                <p class="option">
                    <label><input disabled type="radio" name="<?php echo $name ?>[][option_selected]" value="" /> <?php _e( 'Selected', 'pst' ) ?>
                    </label>
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_error"><?php _e( 'Error Message', 'pst' ) ?></label>
        ...
        <div class="the-metabox checkbox clearfix">
            <label for="<?php echo $id ?>_required"><?php _e( 'Required', 'pst' ) ?></label>
        ...
        <div class="the-metabox checkbox clearfix">
            <label for="<?php echo $id ?>_is_email"><?php _e( 'Email', 'pst' ) ?></label>
        ...
        <div class="the-metabox checkbox clearfix">
            <label for="<?php echo $id ?>_reply_to"><?php _e( 'Reply To', 'pst' ) ?></label>
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_class"><?php _e( 'Class', 'pst' ) ?></label>
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_icon"><?php _e( 'Icon', 'pst' ) ?></label>
            ...
            <div class="option">
                ...
                <div class="input_wrapper custom_icon">
                    ...
                    <input disabled type="button" name="<?php echo $name ?>[][custom]-button" value="<?php _e( 'Upload', 'pst' ) ?>" id="<?php echo $id ?>_icon[custom-icon]-button" class="upload_button button" />
        ...
        <div class="the-metabox text clearfix">
            <label for="<?php echo $id ?>_width"><?php _e( 'Width', 'pst' ) ?></label>
...
<script>
    ...
    jQuery(document).ready(function ($) {
        ...
        $(document).on('click', '#<?php echo $id ?>_addoptions .add-field-option', function(){
            ...
            var option = "<p class='option'><label><input type='radio' name='<?php echo $name ?>[option_selected]' value='' /> <?php _e( 'Selected', 'pst' ) ?></label><input type='text' name='<?php echo $name ?>[" + select_index + "][options][]' style='width:200px' /> <a href='#' class='del-field-option button-secondary'><?php _e( 'Delete option', 'pst' ) ?></a></p>";
        ...
        $(".add-items").click(function () {
            ...
            $('body').trigger('pst_contact_form_added_item');
        ...
        $(document).on('click', '.remove_item', function () {
            if ($('.remove_item').length > 1) {
                ...
                $('body').trigger('pst_contact_form_removed_item');
            }
