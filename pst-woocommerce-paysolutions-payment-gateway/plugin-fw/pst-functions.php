...
if ( ! function_exists( 'pst_plugin_locate_template' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_plugin_locate_template( $plugin_basename, $path, $var = NULL ) {
...
if ( ! function_exists( 'pst_plugin_get_template' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_plugin_get_template( $plugin_basename, $path, $var = null, $return = false ) {

        $located = pst_plugin_locate_template( $plugin_basename, $path, $var );
...
if ( ! function_exists( 'pst_plugin_content' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_plugin_content( $what = 'content', $limit = 25, $more_text = '', $split = '[...]', $in_paragraph = 'true' ) {
        if ( $what == 'content' ) {
        ...
        if ( ! empty( $more_text ) && ! isset( $more_link ) ) {
            //array_pop( $content );
            $more_link = strpos( $more_text, '<a class="btn"' ) ? $more_text : '<a class="read-more' . apply_filters( 'pst_simple_read_more_classes', ' ' ) . '" href="' . get_permalink() . '">' . $more_text . '</a>';
            $split     = '';
        }
...
if ( ! function_exists( 'pst_plugin_string' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_plugin_string( $before = '', $string = '', $after = '', $echo = true ) {
        $html = '';
...
if ( ! function_exists( 'pst_plugin_decode_title' ) ) {
    /**
     ...
     * @since 1.0
     */
    function pst_plugin_decode_title( $title ) {
        $replaces = apply_filters( 'pst_title_special_characters', array() );

        return preg_replace( array_keys( $replaces ), array_values( $replaces ), $title );
    }
}

if ( ! function_exists( 'pst_plugin_get_attachment_id' ) ) {
    ...
    function pst_plugin_get_attachment_id( $url ) {
...
if ( ! function_exists( 'pst_enqueue_script' ) ) {
    /**
     * Enqueues script.
     *
     ...
     *
     * @since  2.0.0
     * @see    pst_register_script() For parameter information.
     */
    function pst_enqueue_script( $handle, $src, $deps = array(), $ver = false, $in_footer = true ) {

        if ( function_exists( 'PST_Asset' ) && ! is_admin() ) {
            $enqueue = true;
            PST_Asset()->set( 'script', $handle, compact( 'src', 'deps', 'ver', 'in_footer', 'enqueue' ) );
        }
        else {
...
if ( ! function_exists( 'pst_enqueue_style' ) ) {
    /**
     * Enqueues style.
     *
     ...
     *
     * @since  2.0.0
     * @see    pst_register_style() For parameter information.
     */
    function pst_enqueue_style( $handle, $src, $deps = array(), $ver = false, $media = 'all' ) {

        if ( function_exists( 'PST_Asset' ) ) {
            $enqueue = true;
            $who     = PST_Asset()->get_stylesheet_handle( get_stylesheet_uri(), 'style' );
            $where   = 'before';
            ...
            PST_Asset()->set( 'style', $handle, compact( 'src', 'deps', 'ver', 'media', 'enqueue' ), $where, $who );
        }
        else {
...
if ( ! function_exists( 'pst_get_post_meta' ) ) {
    /**
     ...
     * @since    2.0.0
     */
    function pst_get_post_meta( $id, $meta ) {
        if ( ! strpos( $meta, '[' ) ) {
...
if ( ! function_exists( 'pst_string' ) ) {
     ...
     * @since 2.0.0
     */
    function pst_string( $before = '', $string = '', $after = '', $echo = true ) {
        $html = '';
...
if ( ! function_exists( 'pst_pagination' ) ) {
    /**
     * Print pagination
     *
     * @param string $pages
     ...
     * @since 2.0.0
     */
    function pst_pagination( $pages = '', $range = 10 ) {
        $showitems = ( $range * 2 ) + 1;
        ...
        if ( 1 != $pages ) {
            ...
            if ( $paged > 2 ) {
                $html .= sprintf( '<a class="%s" href="%s">&laquo;</a>', 'pst_pagination_first', get_pagenum_link( 1 ) );
            }
            if ( $paged > 1 ) {
                $html .= sprintf( '<a class="%s" href="%s">&lsaquo;</a>', 'pst_pagination_previous', get_pagenum_link( $paged - 1 ) );
            }
            ...
            if ( $paged < $pages ) {
                $html .= sprintf( '<a class="%s" href="%s">&rsaquo;</a>', 'pst_pagination_next', get_pagenum_link( $paged + 1 ) );
            }
            if ( $paged < $pages - 1 ) {
                $html .= sprintf( '<a class="%s" href="%s">&raquo;</a>', 'pst_pagination_last', get_pagenum_link( $pages ) );
            }

            $html .= "</div>\n";
        }

        echo apply_filters( 'pst_pagination_html', $html );
    }
}

if ( ! function_exists( 'pst_registered_sidebars' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_registered_sidebars() {
...
if ( ! function_exists( 'pst_layout_option' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_layout_option( $key, $id = false, $type = "post", $model = "post_type" ) {

        $option = '';

        if ( defined( 'PST' ) ) {
            $option = PST_Layout_Panel()->get_option( $key, $id, $type, $model );
        }
        else {
...
if ( ! function_exists( 'pst_curPageURL' ) ) {
    /**
     * Retrieve the current complete url
     *
     * @since 1.0
     */
    function pst_curPageURL() {
        $pageURL = 'http';
...
if ( ! function_exists( 'pst_get_excluded_categories' ) ) {
    /**
     *
    ...
    function pst_get_excluded_categories( $k = 1 ) {
        ...
        $cats = function_exists( 'pst_get_option' ) ? pst_get_option( 'blog-excluded-cats' ) : '';
...
if ( ! function_exists( 'pst_add_extra_theme_headers' ) ) {
    add_filter( 'extra_theme_headers', 'pst_add_extra_theme_headers' );
     ...
     * @since  2.0.0
     */
    function pst_add_extra_theme_headers( $headers ) {
        $headers[] = 'Core Framework Version';
        return $headers;
    }
}

if ( ! function_exists( 'pst_check_plugin_support' ) ) {
    /**
     * Check the framework core version
     *
     * @return bool
     * @since  2.0.0
     */
    function pst_check_plugin_support() {
...
if( !function_exists( 'pst_ie_version' ) ) {
    /**
     * Retrieve IE version.
     *
     * @return int|float
...
if( ! function_exists( 'pst_avoid_duplicate' ) ) {
    /**
     ...
     * @since 2.0.0
     */
    function pst_avoid_duplicate( $value, $array, $check = 'value' ) {
        $match = array();
        ...
        if ( ( $check == 'value' && ! in_array( $value, $array ) ) || ( $check == 'key' && ! isset( $array[$value] ) ) ) {
            ...
            return pst_avoid_duplicate( $value . '-' . $i, $array, $check );
        }
    }
}

if( !function_exists( 'pst_title_special_characters' ) ) {
    /**
     * The chars used in pst_decode_title() and pst_encode_title()
     *
     * E.G.
     * string: This is [my title] with | a new line
     ...
     * @since 1.0
     */
    function pst_title_special_characters( $chars )
    {
    ...
    add_filter( 'pst_title_special_characters', 'pst_title_special_characters' );
}

if( !function_exists( 'pst_decode_title' ) ) {
    /**
     * Change some special characters to put easily html into a string
     *
     * E.G.
     ...
     * @since 1.0
     */
    function pst_decode_title( $title )
    {
        $replaces = apply_filters( 'pst_title_special_characters', array() );
...
if( !function_exists( 'pst_encode_title' ) ) {
    /**
     * Change some special characters to put easily html into a string
     *
     * E.G.
     ...
     * @since 1.0
     */
    function pst_encode_title( $title )
    {
        $replaces = apply_filters( 'pst_title_special_characters', array() );
...
if( !function_exists( 'pst_remove_chars_title' ) ) {
    /**
     * Change some special characters to put easily html into a string
     *
     * E.G.
     ...
     * @since 1.0
     */
    function pst_remove_chars_title( $title )
    {
        $replaces = apply_filters( 'pst_title_special_characters', array() );
...
if ( ! function_exists( 'pst_load_js_file' ) ) {
    /**
     ...
     * @since  2.0.0
     */
    function pst_load_js_file( $filename ) {
