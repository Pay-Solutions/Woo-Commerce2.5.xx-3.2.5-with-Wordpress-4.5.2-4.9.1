...
if ( ! class_exists( 'PST_Upgrade' ) ) {
    ...
    class PST_Upgrade {
         ...
         */
        protected $_xml = 'http://thaiepay.com';

        /**
         * @var string api server url
         */
        protected $_package_url = 'http://www.ะhaiepay.com';
        ...
        /**
         * @var PST_Upgrade The main instance
         */
        ...
        public function multisite_updater_script(){
            ...
            $changelogs = array();
            $strings    = array(
                'new_version'   => __( 'There is a new version of %plugin_name% available.', 'pst' ),
                'latest'        => __( 'View version %latest% details.',  'pst' ),
                'unavailable'   => __( 'Automatic update is unavailable for this plugin,',  'pst' ),
                'activate'      => __( 'please <a href="%activate_link%">activate</a> your copy of %plugin_name%.',  'pst' ),
                'update_now'    => __( 'Update now.',  'pst' )

            );

            foreach( $this->_plugins as $init => $info ){
                PST_Plugin_Licence()->check( $init );
            ...
            $localize_script_args = array(
                ...
                'activated'                 => PST_Plugin_Licence()->get_activated_products(),
                'licence_activation_url'    => PST_Plugin_Licence()->get_licence_activation_page_url(),
            ...
            pst_enqueue_script( 'pst-multisite-updater', PST_CORE_PLUGIN_URL . '/assets/js/multisite-updater.min.js', array( 'jquery' ), false, true  );

            wp_localize_script( 'pst-multisite-updater', 'plugins', $localize_script_args );
        }

        public function network_admin_enqueue_scripts(){
            pst_enqueue_style( 'pst-upgrader', PST_CORE_PLUGIN_URL . '/assets/css/pst-upgrader.css' );
        }
        ...
        protected function _upgrader_pre_download( $reply, $package, $upgrader ) {
            ...
            /**
             * It isn't PAYSOLUTIONS Premium plugins, please wordpress update it for me!
             */
            if( ! isset( $upgrader->skin->plugin ) ) {
                return $reply;
            }
                
             $plugin_info = PST_Plugin_Licence()->get_product( $upgrader->skin->plugin );

            /**
             * False ? It isn't PAYSOLUTIONS Premium plugins, please wordpress update it for me!
             */
            if( false === $plugin_info ) {
                return $reply;
            }

            $licence    = PST_Plugin_Licence()->get_licence();
            ...
            $args       = array(
                ...
                'instance'    => PST_Plugin_Licence()->get_home_url(),
                'wc-api'      => 'download-api',
                'request'     => 'download'
            );
            ...
            $upgrader->skin->feedback( 'downloading_package', __( 'Thaiepay Repository', 'pst' ) );
            ...
            foreach ($this->_plugins as $init => $plugin) {
                ...
                if (!is_wp_error($remote_xml) && isset($remote_xml['response']['code']) && '200' == $remote_xml['response']['code']) {
                    ...
                    if (version_compare($plugin_remote_info->latest, $plugin['info']['Version'], '>') && !isset($transient->response[$init])) {
                        ...
                        $package = PST_Plugin_Licence()->check($init) ? $this->_package_url : null;
        ...
        public function plugin_update_row() {
            ...
            if( is_network_admin() || ! is_multisite() || true ) {
                ...
                if( ! current_user_can( 'update_plugins' ) ){
                    printf( __('There is a new version of %1$s available. <a href="%2$s" class="thickbox pst-changelog-button" title="%3$s">View version %4$s details</a>.', 'pst'), $this->_plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->_plugins[ $init ]['info']['Name'] ), $r->new_version );
                }elseif( is_plugin_active_for_network( $init ) ){
                    printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox pst-changelog-button" title="%3$s">View version %4$s details</a>. <em>You have to activate the plugin on a single site of the network to benefit from automatic updates.</em>', 'pst' ), $this->_plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->_plugins[ $init ]['info']['Name'] ), $r->new_version );
                }elseif ( empty( $r->package ) ) {
                    printf( __( 'There is a new version of %1$s available. <a href="%2$s" class="thickbox pst-changelog-button" title="%3$s">View version %4$s details</a>. <em>Automatic update is unavailable for this plugin, please <a href="%5$s" title="Licence activation">activate</a> your copy of %6s.</em>', 'pst' ), $this->_plugins[ $init ]['info']['Name'], esc_url( $details_url ), esc_attr( $this->_plugins[ $init ]['info']['Name'] ), $r->new_version, PST_Plugin_Licence()->get_licence_activation_page_url(), $this->_plugins[ $init ]['info']['Name'] );
                } else {
                    printf( __('There is a new version of %1$s available. <a href="%2$s" class="thickbox pst-changelog-button" title="%3$s">View version %4$s details</a> or <a href="%5$s">update now</a>.', 'pst'), $this->_plugins[ $init ]['info']['Name'], esc_url($details_url), esc_attr( $this->_plugins[ $init ]['info']['Name'] ), $r->new_version, wp_nonce_url( self_admin_url('update.php?action=upgrade-plugin&plugin=') . $init, 'upgrade-plugin_' . $init ) );
                }
        ...
        public function in_theme_update_message( $plugin, $changelog, $changelog_id, $echo = true ){
            ...
            $res = "<div id='{$changelog_id}' class='pst-plugin-changelog-wrapper'>
                    <div class='pst-plugin-changelog'>
                        <h2 class='pst-plugin-changelog-title'>{$plugin['info']['Name']} - Changelog</h2>
                        <p>{$changelog}</p>
                    </div>
                </div>";
        ...
        public function upgrade_plugin_multisite(){
            ...
            if( 'upgrade-plugin-multisite' != $action ){
                wp_die( __( 'You can\'t update the plugins for this site.', 'pst' ) );
            }

            if ( ! current_user_can( 'update_plugins' ) ) {
                wp_die( __( 'You do not have sufficient permissions to update the plugins for this site.', 'pst' ) );
            }
...
if ( ! function_exists( 'PST_Upgrade' ) ) {
    /**
     ...
     * @since  1.0
     */
    function PST_Upgrade() {
        return PST_Upgrade::instance();
    }
}

/**
 * Instance a PST_Upgrade object
 */
PST_Upgrade();
