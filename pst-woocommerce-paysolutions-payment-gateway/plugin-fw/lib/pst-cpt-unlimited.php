...
class PST_CPT_Unlimited {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( $this->_args['add_multiuploader'] ) {
            ...
            add_action( 'wp_ajax_pst_cptu_multiuploader', array( $this, 'post_multiuploader' ) );
        }
        ...
        if ( $this->_args['has_single'] ) {
            add_action( 'pst_loop', array( $this, 'single_template' ) );
        ...
        if ( ! empty( $this->_args['shortcode_name'] ) ) {
            ...
            add_filter( 'pst_shortcode_' . $this->_args['shortcode_name'] . '_icon', array( $this, 'shortcode_icon') );
            add_filter( 'pst-shortcode-plugin-init', array( $this, 'add_shortcode_to_box' ) );
        }
	...
	public function admin_sortable_assets() {
		...
		wp_enqueue_script( 'pst-cptu-sortable-posts', PST_CORE_PLUGIN_URL . '/assets/js/pst-cptu-sortable-posts.js', array( 'jquery', 'jquery-ui-sortable' ), '1.0', true );
	}
    ...
    public function register_post_type() {
        $labels = array(
            ...
            'add_new'            => sprintf( __( 'Add %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ),
            'add_new_item'       => sprintf( __( 'Add New %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ),
            'edit_item'          => sprintf( __( 'Edit %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ),
            'new_item'           => sprintf( __( 'New %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ),
            'all_items'          => sprintf( __( 'All %s', 'pst' ), ucfirst( $this->_labels['plural'] ) ),
            'view_item'          => sprintf( __( 'View %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ),
            'search_items'       => sprintf( __( 'Search %s', 'pst' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found'          => sprintf( __( 'No %s found', 'pst' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'pst' ), ucfirst( $this->_labels['plural'] ) ),
            'parent_item_colon'  => '',
            'menu_name'          => ucfirst( $this->_labels['menu'] )
        );

        $args = array(
            'labels'             => apply_filters( 'pst_' . $this->_name . '_labels', $labels ),
            'public'             => false,
        ...
        register_post_type( $this->_name, apply_filters( 'pst_' . $this->_name . '_args', $args ) );
    }
    ...
    protected function _cpt_args( $post ) {
        ...
        $args = apply_filters( 'pst_cptu_register_post_type_args', array(
    ...
    public function register_cptu_post_types() {
        ...
        foreach ( $post_types as $pt ) {
            ...
            $labels = array(
                ...
                'add_new'            => sprintf( __( 'Add %s', 'pst' ), $label_singular ),
                'add_new_item'       => sprintf( __( 'Add New %s', 'pst' ), $label_singular ),
                'edit_item'          => sprintf( __( 'Edit %s', 'pst' ), $label_singular ),
                'new_item'           => sprintf( __( 'New %s', 'pst' ), $label_singular ),
                'all_items'          => sprintf( __( 'All %s', 'pst' ), $label_plural ),
                'view_item'          => sprintf( __( 'View %s', 'pst' ), $label_singular ),
                'search_items'       => sprintf( __( 'Search %s', 'pst' ), $label_plural ),
                'not_found'          => sprintf( __( 'No %s found', 'pst' ), $label_plural ),
                'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'pst' ), $label_plural ),
                'parent_item_colon'  => '',
                'menu_name'          => $title
            );

            $args = array(
                'labels'             => apply_filters( 'pst_' . $this->_prefix_cpt . $name . '_labels', $labels ),
                'public'             => true,
            ...
            // register post type
            $post_type = pst_avoid_duplicate( str_replace( '-', '_', substr( $this->_prefix_cpt . $name, 0, 16) ), $post_types );
            register_post_type( $post_type, apply_filters( 'pst_' . $this->_prefix_cpt . $name . '_args', $args, $pt ) );  // save the post type in post meta
            ...
            if ( $this->_args['has_taxonomy'] && ! empty( $taxonomy ) ) {
                ...
                $labels = array(
                    'name'              => sprintf( _x( '%s Categories', 'taxonomy general name', 'pst' ), $label_singular ),
                    'singular_name'     => _x( 'Category', 'taxonomy singular name', 'pst' ),
                    'search_items'      => __( 'Search Categories', 'pst' ),
                    'all_items'         => __( 'All Categories', 'pst' ),
                    'parent_item'       => __( 'Parent Category', 'pst' ),
                    'parent_item_colon' => __( 'Parent Category:', 'pst' ),
                    'edit_item'         => __( 'Edit Category', 'pst' ),
                    'update_item'       => __( 'Update Category', 'pst' ),
                    'add_new_item'      => __( 'Add New Category', 'pst' ),
                    'new_item_name'     => __( 'New Category Name', 'pst' ),
                    'menu_name'         => __( 'Category', 'pst' ),
                );
        ...
        wp_cache_set( 'pst_cptu_post_types', $post_types );
    }
    ...
    public function add_cptu_menu_item() {
        ...
        if ( strpos( $post_type, $this->_prefix_cpt ) !== false ) {
            ...
            $submenu[ $item ][20] = array( sprintf( __('Add %s', 'pst'), ucfirst( $label_singular ) ), 'edit_posts', 'post-new.php?post_type=' . $post_type );

            global $wp_taxonomies;
            $taxonomy = get_post_meta( $portfolio->ID, '_taxonomy', true );
            if ( isset( $wp_taxonomies[ $taxonomy ] ) ) {
                $submenu[ $item ][25] = array( __('Categories', 'pst'), 'edit_posts', 'edit-tags.php?taxonomy=' . $taxonomy . '&post_type=' . $post_type );
            }
        }
    ...
    public function frontend_assets() {
        ...
        if ( ! is_single() || ! isset( $post->post_type ) || ! $this->_is_valid( $post->post_type ) ) {
            ...
            foreach ( $posts as $post_id ) {
                ...
                foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                    ...
                    pst_enqueue_style( $asset['handle'], empty( $asset['src'] ) ? false : $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
                }
        ...
        // load assets of single template
            ...
            foreach ( $this->layouts[ $layout ]['css'] as $asset ) {
                ...
                pst_enqueue_style( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['media'] );
            }
        }
    ...
    public function add_metabox_cptu() {
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            $layouts = apply_filters( 'pst_cptu_' . $this->_name . '_layout_values', $layouts );
        }

        $single_layouts = apply_filters( 'pst_cptu_' . $this->_name . '_single_layout_values', array() );

        $metabox_args = array(
            'label'    => sprintf( __( '%s Settings', 'pst' ), $this->_labels['singular'] ),
            'pages'    => $this->_name, //or array( 'post-type1', 'post-type2')
            'context'  => 'normal', //('normal', 'advanced', or 'side')
            'priority' => 'default',
            'tabs'     => array(
                'settings' => array(
                    'label'  => __( 'Settings', 'pst' ),
                    'fields' => apply_filters( 'pst_cptu_fields', array(
                        'type'      => array(
                            'label' => __( 'Type', 'pst' ),
                            'desc'  => __( 'Layout for this '.strtolower( $this->_labels['singular'] ) , 'pst' ),
                            'type'  => 'select',
                            'options' => isset( $layouts ) ? $layouts : array(),
                            'std'   => '' ),

                        'rewrite'     => array(
                            'label' => __( 'Rewrite', 'pst' ),
                            'desc'  => __( 'Univocal identification name in the URL for each product (slug from post if empty)', 'pst' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_singular' => array(
                            'label' => __( 'Label in Singular', 'pst' ),
                            'desc'  => __( 'Set a label in singular (title of portfolio if empty)', 'pst' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_plural' => array(
                            'label' => __( 'Label in Plural', 'pst' ),
                            'desc'  => __( 'Set a label in plural (title of portfolio if empty)', 'pst' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy' => array(
                            'label' => __( 'Taxonomy', 'pst' ),
                            'desc'  => __( 'If you want to use categories in the portfolio, set a name for taxonomy. Name should be a slug (must not contain capital letters nor spaces) and must not be more than 32 characters long (database structure restriction).', 'pst' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy_rewrite' => array(
                            'label' => __( 'Taxonomy Rewrite', 'pst' ),
                            'desc'  => __( 'Set univocal name for each category page URL.', 'pst' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'single_layout' => array(
                            'label' => __( 'Single layout', 'pst' ),
                            'desc'  => __( 'Layout for single page of this portfolio', 'pst' ),
                            'type'  => 'select',
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            foreach ( $this->layouts as $layout => $args ) {
                ...
                $metabox_args['tabs']['settings']['fields'][ $layout . '_title' ] = array(
                    'desc' => $args['name'] . ' ' . __( 'layout settings', 'pst' ),
                    'type' => 'title',
                    'deps' => array(
                        'ids' => '_type',
                        'values' => $layout
        ...
        $metabox = PST_Metabox( $this->_name . '_cptu_settings' );
    ...
    public function add_metabox_item_fields() {
        ...
        $metabox = PST_Metabox( $post_type . '_item_fields' );
        $metabox->init( $metabox_args );
    ...
    public function add_quick_links_metaboxes() {
        // CPTU
        add_meta_box( $this->_name . '_quick_links', __( 'Quick links', 'pst' ), array( $this, 'quick_links_cptu_inner' ), $this->_name, 'side', 'high' );
        ...
        foreach ( $post_types as $post ) {
            ...
            add_meta_box( $post->post_type . '_quick_links', __( 'Quick links', 'pst' ), array( $this, 'quick_links_cpt_inner' ), $post_type, 'side', 'high' );
        }
    }
    ...
    public function quick_links_cptu_inner( $post ) {
        extract( $this->_cpt_args( $post ) );
        ?>
        <a href="<?php echo admin_url( 'edit.php?post_type=' . get_post_meta( $post->ID, '_post_type', true ) ) ?>"><?php printf( __( 'View %s', 'pst' ), $label_plural ) ?></a>
        <?php
    }
    ...
    public function quick_links_cpt_inner( $post ) {
        ...
        ?>
        <a href="<?php echo admin_url( "post.php?post={$post->ID}&action=edit" ) ?>"><?php printf( __( 'Edit %s', 'pst' ), $post->post_title ) ?></a>
        <?php
    }
    ...
    public function cptu_change_columns( $column, $post_id ) {
        ...
        switch ( $column ) {
            case 'actions' :
                echo '<a href="' . admin_url( "post.php?post={$post_id}&action=edit" ) . '" class="button-secondary">' . sprintf( __( 'Edit %s', 'pst' ), ucfirst( $this->_labels['singular'] ) ) . '</a> ';
                echo '<a href="' . admin_url( 'edit.php?post_type=' . get_post_meta( $post_id, '_post_type', true ) ) . '" class="button-secondary">' . sprintf( __( 'View %s', 'pst' ), $label_plural ) . '</a> ';
                break;
        }
    }
    ...
    public function add_shortcode( $atts, $content = null ) {
        ...
        if( function_exists( 'PST_Portfolio' ) && method_exists( PST_Portfolio(), 'is' ) && PST_Portfolio()->is( $atts['name'] ) ){
            $atts['name'] = str_replace( PST_Portfolio()->post_type_prefix, '', $atts['name'] );
        }
        ...
        extract( apply_filters( 'pst_cptu_frontend_vars', $atts, $this->_name ) );
        ...
        if ( $this->_args['manage_layouts'] && isset( $this->layouts[ $layout ]['js'] ) && ! empty( $this->layouts[ $layout ]['js'] ) ) {
            foreach ( $this->layouts[ $layout ]['js'] as $asset ) {
                ...
                pst_enqueue_script( $asset['handle'], $this->locate_url( $layout ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
     ...
     *
     * @param $icon_url string Icone url found by pst_shortcode plugin
     *
     * @return string
     * @since 1.0.0
     * @author Wilawan Onnom' <wilawan@efrainc.com>
     ...
     *
     * All definition settings to add cptu shortcode to PST Shortcode Panel
     *
    ...
    public function add_shortcode_to_box( $shortcodes ) {
        ...
        $args = array(
            $this->_args['shortcode_name'] => array(
                ...
                'description' => sprintf( __( 'Show frontend of the %s', 'pst' ), $this->_labels['main_name'] ),
                'tab'         => 'cpt',
                'create'      => false,
                'has_content' => false,
    ...
    */
    ...
    public function admin_assets() {
        wp_enqueue_media();
        wp_enqueue_script( 'pst-cptu', PST_CORE_PLUGIN_URL . '/assets/js/pst-cpt-unlimited.js', array('jquery'), '', true );
    }
        ...
        <script>
            (function($) {
                ...
                var button = $('<a />', {
                    ...
                    'data-uploader_title': '<?php printf( __( 'Add %s from images', 'pst' ), $label_plural ) ?>',
                    'data-uploader_button_text': '<?php printf( __( 'Add %s', 'pst' ), $label_plural ) ?>'
                }).text('<?php _e( 'Add with multiupload', 'pst' ) ?>');
...
//        }
        ...
        if ( $this->_args['manage_layouts'] ) {
            foreach ( $this->layouts[ 'single' ]['js'] as $asset ) {
                pst_enqueue_script( $asset['handle'], $this->locate_url( 'single' ) . $asset['src'], $asset['deps'], $asset['ver'], $asset['in_footer'] );
            }
        }
        ...
        extract( apply_filters( 'pst_cptu_frontend_vars', array( 'name' => $post_name ), $this->_name ) );
    ...
    public function archive_template() {
        ...
        remove_action( 'pst_content_loop', 'pst_content_loop', 10 );
        add_action( 'pst_content_loop', array( $this, 'archive_template_loop' ), 10 );
