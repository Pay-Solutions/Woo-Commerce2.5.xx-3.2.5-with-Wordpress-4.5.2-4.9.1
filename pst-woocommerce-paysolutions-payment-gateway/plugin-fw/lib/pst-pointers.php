    ...
    class PST_Pointers {
        ...
        public function __construct() {
            ...
            $this->_default_pointer['plugins'] = array(
                'screen_id'  => 'plugins',
                'options'    => array(
                    'content'  => sprintf( '<h3> %s </h3> <p> %s </p> <p> %s <a href="http://thaiepay.com/download.aspx" target="_blank">Paysolutions.com</a> %s
                                  <a href="https://profiles.wordpress.org/" target="_blank">Wordpress.org</a></p>',
                                __( 'Plugins Activated', 'pst' ),
                                __( 'From now on, you can find all plugin options in PAYSOLUTIONS Plugin menu.
                                     For each plugin installed, customization settings will be available as a new entry in PAYSOLUTIONS Plugin menu.', 'pst' ),
                                __( 'Discover all our plugins available on:', 'pst' ),
                                __( 'and', 'pst' )
                    ),
                ),
            );
            ...
            $this->_default_pointer['update'] = array(
                ...
                'options'    => array(
                    'content'  => sprintf( '<h3> %s </h3> <p> %s </p> <p> %s <a href="http://thaiepay.com/download.aspx" target="_blank">Paysolutions.com</a> %s
                                  <a href="https://profiles.wordpress.org/" target="_blank">Wordpress.org</a></p>',
                                __( 'Plugins Upgraded', 'pst' ),
                                __( 'From now on, you can find all options of your plugins in PAYSOLUTIONS Plugin menu.
                                    Any time one of our plugins is updated, a new entry will be added to this menu.
                                    For example, after the update, plugin options (such as for PAYSOLUTIONS WooCommerce Wishlist, PAYSOLUTIONS WooCommerce Ajax Search, etc.)
                                    will be moved from previous location to PAYSOLUTIONS Plugin tab.', 'pst' ),
                                __( 'Discover all our plugins available on:', 'pst' ),
                                __( 'and', 'pst' )
                    ),
                ),
             ...
             */
            $this->special_screen = apply_filters( 'pst-pointer-special-screen', array( 'plugins', 'update' ) );
        ...
        public function parse_args( $args ) {
            $default = array(
                'pointer_id' => 'payh_default_pointer',
        ...
        public function add_pointers(){
            if( ! empty( $this->screen_ids ) ){
                foreach( $this->screen_ids as $screen_id ){
                    add_filter( "pst_pointers-{$screen_id}", array( $this, 'pointers' ) );
                }
            }
        }
        ...
        public function pointer_load( $hook_suffix ) {
            ...
            $pointers   = apply_filters( "pst_pointers-{$screen->id}", array() );
            ...
            if( in_array( $screen->id, $this->special_screen ) ){
                ...
                $recently_activate  = get_option( 'pst_recently_activated', array() );
                ...
                if( $is_single_activate || $is_single_upgrade ){
                    ...
                    if( $is_multi_upgrade && isset( $_POST['checked'] ) && ( count( $_POST['checked'] )  > 0 ) ){
                        ...
                        $screen_id          = 'update';
                        $pointers           = apply_filters( "pst_pointers-{$screen_id}", array() );
                    }
                     ...
                     *
                     * count( $point_id ) is the number of PAYSOLUTIONS plugins that have registered specific pointers
                ...
                update_option( 'pst_recently_activated', array() );
            ...
            $script_file = function_exists( 'pst_load_js_file' ) ? pst_load_js_file( 'pst-wp-pointer.js' ) : 'pst-wp-pointer.min.js';
            ...
            wp_enqueue_script( 'pst-wp-pointer', PST_CORE_PLUGIN_URL . '/assets/js/' . $script_file, array( 'wp-pointer' ), false, true );
            wp_localize_script( 'pst-wp-pointer', 'custom_pointer', $valid_pointers );
        }

        public function pointers( $pointers ){
            $screen_id          = str_replace( 'pst_pointers-', '', current_filter() );
...
if ( ! function_exists( 'PST_Pointers' ) ) {
    /**
     * Main instance of plugin
     *
     * @return object PST_Pointers
     * @since  1.0
     */
    function PST_Pointers() {
        return PST_Pointers::instance();
    }
}

/**
 * Instance a PST_Pointers object
 */
PST_Pointers();